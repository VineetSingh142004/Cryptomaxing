...
import { getAllPublicProviderTests } from "@/lib/vault/public-provider-store";import type { RunProviderContributions } from "@/lib/trading/paper/provider-contribution";
...
function resolveLunarCrushConnection(env: ReturnType<typeof getProviderEnvSettings>): {
  connectionStatusLabel: ProviderConnectionStatus;
  status: string;
  ...
  if (!env.lunarcrushEnabled) {
    return {
      connectionStatusLabel: "DISABLED",
      status: "DISABLED",
      currentRunReason: "Disabled by config (LUNARCRUSH_ENABLED≠true)",
      currentRunContribution: "DISABLED",
    };
  }
  if (!env.lunarcrushApiKey) {
    return {
      connectionStatusLabel: "UNKNOWN",
      status: "MISSING_API_KEY",
      currentRunReason: "Missing API key (LUNARCRUSH_API_KEY or vault credential)",
      currentRunContribution: "NOT_USED",
    };
  }
  return {
    connectionStatusLabel: "READY_WITH_KEY",
    status: "READY_WITH_KEY",
    currentRunReason: "Not wired into scanner pipeline — optional enrichment only",
    currentRunContribution: "SKIPPED_BY_PIPELINE",
  };
}
...
export function buildScannerProviderStatus(input?: {
  ...
  const providers: ScannerProviderStatusEntry[] = [
    ...
    },
    {
      ...
      label: "LunarCrush",
      ...(() => {
        const lc = resolveLunarCrushConnection(env);
        const runStatus = input?.lunarcrushStatus;
        let contribution = lc.currentRunContribution;
        let reason = lc.currentRunReason;
        if (runStatus === "unavailable") {
          contribution = "FAILED_THIS_RUN";
          reason = "API failed this run — scanner continues without LunarCrush";
        ...
        return {
          ...
          connectionStatus:
            lc.connectionStatusLabel === "DISABLED"
              ? "DISABLED"
              : lc.connectionStatusLabel === "UNKNOWN"
                ? "UNKNOWN"
                : "OK",
          connectionStatusLabel: lc.connectionStatusLabel,
          currentRunContribution: contribution,
          currentRunReason: reason,
          lastTestedAt: null,
          endpointTested: null,
        };
      })(),
